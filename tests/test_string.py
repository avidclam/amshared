...
def test_namedlov():
    ...
    with pytest.raises(TypeError, match=r"required positional"):
        string.NamedLoV()
    assert string.NamedLoV(x0).data == x0
    assert string.NamedLoV(x0).export() == [{nm: lv} for nm, lv in x0.items()]
    for x in (x1, x2, x3, x4):
        assert string.NamedLoV(x, sep=',').data == x0
    nlv = string.NamedLoV(x4)
